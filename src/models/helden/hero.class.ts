...
export function createHero(id: HeroId, useExtraDeck = false): Hero {
  ...
  hero.cardstack = hero.buildCardstack(
    useExtraDeck ? mergeWithExtraDeck(definition) : definition.cardCounts,
  );
  return hero;
}

function mergeWithExtraDeck(definition: HeroDefinition): Map<string, number> {
  const extraDefinition = HERO_DEFINITIONS.find(
    (def) => def.id === EXTRA_DECK_FOR_HERO[definition.id],
  )!;
