...
export class Monster {
  ...
  createMob(
    ...
    const bossIndex = BOSS_ORDER.includes(currentBossName)
      ? BOSS_ORDER.indexOf(currentBossName)
      : BOSS_ORDER.length;
    const difficultyIndex = DIFFICULTY_INDEX[difficulty] ?? 2;

    const monsterCount =
      2 * numberOfPlayers + 6 + 4 * bossIndex + 4 * difficultyIndex;
  ...
  loadMonster(numberOfMonsterCards: number) {
    const monsterCollectionCopy = [...this.monsterCollection];
  ...
  loadQuests(numberOfQuestCards: number) {
    const questCollectionCopy = [...this.questCollection];
  ...
  loadSoloQuests(numberOfQuestCards: number) {
    const questCollectionCopy = this.questCollection.filter(
      (quest) => quest.name !== 'Chaos',
    );
    const count = Math.min(numberOfQuestCards, questCollectionCopy.length);
    for (let i = 0; i < count; i++) {
      const randomIndex = Math.floor(
