...
describe('MagischeBombeEffect', () => {
  ...
  it('removes one occurrence of each present symbol and reports the resulting threat', async () => {
    let enemy = { name: 'Zola, die Gorgone', type: 'Boss', token: ['red', 'red', 'yellow', 'purple'] };
    ...
    expect(ctx.updateCurrentEnemyToken).toHaveBeenCalledWith(jasmine.objectContaining({ token: ['red'] }));
    expect(ctx.checkForNextEnemy).toHaveBeenCalledWith(jasmine.objectContaining({ token: ['red'] }));
    expect(ctx.saveHand).toHaveBeenCalledWith('magischeBombe', ['magischeBombe']);
