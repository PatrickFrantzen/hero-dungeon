...
describe('GameFactoryService', () => {
  ...
  it('buildNewGame creates ten pre-boss encounters for a singleplayer game on Lehrling-Schwierigkeit (Issue #86)', () => {
    ...
    expect(
      preBossEncounters.filter((entry) =>
        NORMAL_ENEMY_TYPES.includes(entry.type),
      ).length,
    ).toBe(8);
    expect(
      preBossEncounters.filter(
        (entry) => !NORMAL_ENEMY_TYPES.includes(entry.type),
      ).length,
    ).toBe(2);
    expect(
      preBossEncounters.some((entry) => entry.name === 'Chaos'),
    ).toBeFalse();
  });
});
