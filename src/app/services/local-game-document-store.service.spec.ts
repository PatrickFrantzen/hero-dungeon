...
describe('LocalGameDocumentStoreService', () => {
  ...
  it('setDoc on a game path creates the save, readable via getDoc', () => {
    service.setDoc(['games', 'local-1'], {
      gameId: 'local-1',
      numberOfPlayers: 1,
    });

    expect(service.getDoc(['games', 'local-1'])).toEqual({
      gameId: 'local-1',
      numberOfPlayers: 1,
    });
  });
  ...
  it('setDoc on a player path is readable via getDoc without touching the game doc', () => {
    ...
    service.setDoc(['games', 'local-1', 'player', 'solo'], {
      userId: 'solo',
      handstack: [],
    });
  ...
  it('updateFields merges a patch into the player doc instead of replacing it', () => {
    ...
    service.updateFields(['games', 'local-1', 'player', 'solo'], {
      handstack: ['blue'],
    });
